...
function createChannel() {
  ...
  scripts/createChannel-mod.sh $CHANNEL_NAME $CLI_DELAY $MAX_RETRY $VERBOSE $APP_PROFILE $ORG
}


## Call the script to deploy a chaincode to the channel
function deployCC() {
  scripts/deployCC-mod.sh $CHANNEL_NAME $CC_NAME $CC_SRC_PATH $CC_SRC_LANGUAGE $CC_VERSION $CC_SEQUENCE $CC_INIT_FCN $CC_END_POLICY $CC_COLL_CONFIG $CLI_DELAY $MAX_RETRY $VERBOSE $ORG
...
APP_PROFILE="SingleOneOrgsApplicationGenesis"
ORG=1
